...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	function test_locate_plugin() {
		...
		$this->assertEquals( 'customize-concurrency', $location['dir_basename'] );
		$this->assertContains( 'plugins/customize-concurrency', $location['dir_path'] );
		$this->assertContains( 'plugins/customize-concurrency', $location['dir_url'] );
	}
	...
	function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use( $obj ) {
			$obj->assertEquals( 'CustomizeConcurrency\Plugin: Param is 0!', $errstr );
